...
LightHandler::LightHandler(uint8_t *pins, float *_white, float *_voff, bool *_motionFlag) : white(*_white),
                                                                                            voff(*_voff),
                                                                                            pinPtr(pins),
...
void LightHandler::handleHsvImpl(HttpRequest &request, HttpResponse &response)
{
    ...
    if (paramV.length() > 0)
    {
        if (request.getQueryParameter("v_new").length() > 0)
        {
            ...
            if (request.getQueryParameter("v_new").toFloat() >= 0 && request.getQueryParameter("v_new").toFloat() <= 100)
            {
    ...
    {
        if (request.getQueryParameter("v_new").length() > 0 && request.getQueryParameter("v_new").toFloat() >= 0.0 && request.getQueryParameter("v_new").toFloat() <= 100.0)
        {
            v_new = request.getQueryParameter("v_new").toFloat();
        }
    }
